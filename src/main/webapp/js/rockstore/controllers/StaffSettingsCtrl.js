...
allControllers.controller('StaffSettingsCtrl', ['$scope','$rootScope','$http','DropDownValueService','$filter','spinnerService','modalService',
	...
	$scope.submit = function(){
		if(!$scope.form.email && !$scope.userForm.$valid){
			modalService.showModal({}, {    	            	           
		           headerText: "Invalid Email entry",
		           bodyText: "Check that you have entered a valid email address"
	    	 });
			return;
		}
		if(!($scope.form.contactName && $scope.form.organization)){
			 modalService.showModal({}, {    	            	           
		           headerText: "Invalid Entry",
		           bodyText: "Check that you have a unique username and organization field is not blank."
	    	 });
			 return;
		}
